...
export default {
  ...
  render(h) {
    ...
    return h(
      ...
      "<colgroup>{this.columns.map(column => <col name={ column.id } key={column.id} />)}
      {this.hasGutter ? "<col name='gutter' /> : ''}</colgroup><thead class={ [{ 'is-group': isGroup, 'has-gutter': this.hasGutter }] }>{this._l(columnRows, (columns, rowIndex) => <tr style={ this.getHeaderRowStyle(rowIndex) } class={ this.getHeaderRowClass(rowIndex) }>"+columns.map((column, cellIndex) => ("<th colspan={ column.colSpan }
